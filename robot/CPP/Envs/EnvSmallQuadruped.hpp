...
#ifndef EnvSmallQuadruped_hpp
#define EnvSmallQuadruped_hpp

#include  "Env.hpp"


class SmallQuadrupedEnv:public MjEnv
{
public:
    std::uint16_t m_act_size = 8;
    std::uint16_t m_ob_size = 20;
    SmallQuadrupedEnv(std::string name, int m_seed=0, int step_limit=200);
    ~SmallQuadrupedEnv();
...
private:
    ...
    std::string m_name = "smallquadruped";
};

#endif /* SmallQuadruped_hpp */
